...
async function scrapeblkbrdshoemakerProduct(url) {
  ...
  timeout: 60000,
    ...
    const data = await page.evaluate(() => {
      const product_name =
        document.querySelector("h1.ProductMeta__Title")?.innerText.trim() ||
        "N/A";

      const rawPrice =
        document.querySelector("span.money")?.innerText.trim() || "";
      const product_price = rawPrice.replace(/[^\d]/g, "") || "N/A";

      const imgElement = document.querySelector(
        ".Product__Slideshow img.Image--lazyLoaded"
      );
...
async function scrapeNykaaProduct(url) {
  ...
  try {
    ...
    await page
      .waitForSelector(".css-z904mr", { timeout: 3000 })
      .catch(() => null);
    await page
      .waitForSelector(".css-a5kl1t", { timeout: 3000 })
      .catch(() => null);

    const data = await page.evaluate(() => {
      const name =
        document.querySelector(".css-ef32ai")?.innerText.trim() || "N/A";

      let priceSymbol = document.querySelector(".css-a5kl1t")?.innerText || "₹";
      let priceDigits =
        document
          .querySelector(".css-a5kl1t")
          ?.nextSibling?.textContent?.trim() || "";
      let product_price =
        (priceSymbol + priceDigits).replace(/[^\d]/g, "") || "N/A";
...
async function myntra(url) {
  ...
  '(KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36'
    ...
    database = {
      ...
      product_image_url: data.style.media.albums[0].images[0].imageURL,
      product_price:
        data?.style?.sizes?.[0]?.sizeSellerData?.[0]?.discountedPrice ??
        data?.style?.mrp,
    };
    // console.log("there");
    // }
...
export async function POST(request) {
  ...
  if (domain === "amazon") {
    ...
    data = await scrapeAmazonProduct(url.link);
    if (data) {
    ...
    data = await scrapeblkbrdshoemakerProduct(url.link);
    if (data) {
      data = JSON.parse(data);
    } else {
      console.error("❌ Failed to get data from scraper.");
    ...
    data = await scrapeNykaaProduct(url.link);
    if (data) {
      data = JSON.parse(data);
    } else {
      console.error("❌ Failed to get data from scraper.");
    ...
    if (data) {
      ...
      data = await luxurytatacliq(url.link);
      if (data) {
        data = JSON.parse(data);
      } else {
        console.error("❌ Failed to get data from scraper.");
    ...
    data = await extractFlipkartProductData(url.link);
    if (data) {
      data = JSON.parse(data);
    } else {
      console.error("❌ Failed to get data from scraper.");
    ...
    data = await getAdidasProductData(url.link);
    if (data) {
      data = JSON.parse(data);
    } else {
      console.error("❌ Failed to get data from scraper.");
  ...
  try {
    ...
    if (
      ...
      console.warn("⚠️ Skipping DB update because data is empty or invalid.");
